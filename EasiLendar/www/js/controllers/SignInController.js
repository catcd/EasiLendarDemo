		...
		.controller(
				...
				function($rootScope, $scope, $timeout, $http, $state,
					...
					function User() {
						...
						/*
						 * check ID Can not be empty 
						 * ID must be unique (check later when connect to server)
						 */
						...
						/*
						 * check confirm password Can not be empty Must match
						 * with password
						 */
					 ...
					 * (!$scope.user.checkChar()) { $state.go('warning'); } else {
					 * var id = $scope.user.id; var pass = $scope.user.password;
					 * $http.post("php/signIn.php", {"ID": id, "pass": pass})
					 * .success(function(data,status,headers,config) { if (data ==
					 * "YES") { $state.go(link); } else { $state.go(link); //
					 * should be warning } }) .error(function(data,status) {
					...
					$scope.register = function() {
						// reset all warnings
						for (var i=0; i < 5; i++) { $scope.warnings.reset(i); }
						...
						// if inputs are correct (or else all users will be licked)
						if (!check) { 
