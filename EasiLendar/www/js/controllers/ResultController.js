...
result.controller('ResultController', function($rootScope, $scope, eSettings,
	eFriend, eMultiCalendar, eEasiLendar, eSAlgorithm, eSearchFilter, eUser) {
	...
	if (eFriend.fMultiCal !== null) {
		// the multiCalendar combine this user calendar
		// with user's friend's calendar
		$rootScope.resultMultiCalendar = eMultiCalendar.newMultiCal([
			eUser.uGmailCalendar, eFriend.fMultiCal.calendar]);
	} else {
		$rootScope.resultMultiCalendar = eMultiCalendar.newMultiCal([
		...
		next : function() {
			...
			if ($scope.weekCalendar.navDays[0].origin.toDate() >= new Date()) {
				$scope.navStart = $scope.weekCalendar.navDays[0].origin.
																	toDate();
				$scope.navEnd = $scope.weekCalendar.navDays[6].origin.toDate();
				$scope.mHeap = eSAlgorithm.evaluateTime(
								$rootScope.resultMultiCalendar, $scope.navStart,
								$scope.navEnd, eSearchFilter.mDuration);
			} else if ($scope.weekCalendar.navDays[6].origin.toDate() >
																new Date()) {
				var date = new Date();	// today
				$scope.navStart = new Date(date.getFullYear(), date.getMonth(),
											date.getDate());
				$scope.navEnd = $scope.weekCalendar.navDays[6].origin.toDate();
				$scope.mHeap = eSAlgorithm.evaluateTime(
					$scope.resultMultiCalendar, $scope.navStart, $scope.navEnd,
					eSearchFilter.mDuration);
			} else {
				$scope.mHeap = null;
			}
