...
.controller('MyProfileController',
	function($scope, $rootScope, $ionicSlideBoxDelegate, $ionicPopup,
		...
		var todayStatus = function() {
			var temp = new Date();
			var startToday = new Date(temp.getFullYear(),
				temp.getMonth(),
				temp.getDate());
			var check = false;

			if (eUser.uGmailCalendar !== null &&
				eUser.uGmailCalendar[startToday] !== undefined) {
				...
				data.every(function(element) {
					if (element.start.dateTime > temp) {
						...
						document.getElementById('my-profile-status')
							.innerHTML = 'Next event: ' + element.summary +
							'<br>' + eCalendar.convertTime(element) +
							(element.location !== undefined ? '<br>at ' +
								element.location : '');
						check = true;
						return false;
					}
					if (!check) {
						document.getElementById('my-profile-status')
							.innerHTML = 'Today has nothing to do left.';
					}
				});
			} else {
				document.getElementById('my-profile-status')
		...
		var updateTime = function() {
			...
			document.getElementById('my-profile-date')
				.innerHTML = $scope.timeInfo.date;
			document.getElementById('my-profile-time')
				.innerHTML = $scope.timeInfo.time;
		};
		...
		$scope.done = function() {
			if ($scope.data.year || $scope.data.month || $scope.data.date) {
				$scope.data.year =
					($scope.data.year === null ? '1990' : $scope.data.year);
				$scope.data.month =
					($scope.data.month === null ? '1' : $scope.data.month);
				$scope.data.date =
					($scope.data.date === null ? '1' : $scope.data.date);

				eUser.uBirthday = new Date(parseInt($scope.data.year),
					parseInt($scope.data.month) - 1,
					parseInt($scope.data.date));
			}
