...
.controller('SyncController', function($scope, $rootScope, $window, $document, eSync, eUser, $cordovaCalendar) {
	...
	$scope.buttonAffect = function() {
		...
		if ($scope.logIN == 0) {
			...
			if ($scope.email != '') {
				...
				hello.innerHTML = "Hi, " + $scope.email;
			}
		...
		if ($scope.logIN == 1) {
			...
			gapi.client.load('calendar', 'v3', function() {
				...
				request.execute(function(resp) {
					...
					$scope.eUser.uGmailCalendar = resp.items;

					$scope.eSync.convertMe();
