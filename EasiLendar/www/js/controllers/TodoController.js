...
.controller('TodoController',
	function($scope, $rootScope,
		...
		eUser, eToast, eDatabase, eTodo) {
		// inject services
		$scope.eUser = eUser;
		$scope.eDatabase = eDatabase;
		eUser.uTodo = eTodo.tChecklist;
		...
		$scope.countTask = function() {
			...
			eUser.uTodo.forEach(function(element) {
				if (!element.done) {
					element.list.forEach(function(elementIn) {
						if (!elementIn.done) {
		...
		$scope.editChecklist = function(checklist) {
			$scope.data.newName =
				eUser.uTodo[$scope.findIndexOf(checklist)].listName;
				...
				buttons: [{
					...
					onTap: function(e) {
						if ($scope.data.newName === '' ||
							$scope.data.newName === undefined) {
							e.preventDefault();
						} else {
							eUser.uTodo[$scope.findIndexOf(checklist)]
								.listName = $scope.data.newName;
							$scope.makeChangeTodoList();
							$scope.data = {};
						}
					}
		...
		$scope.createTask = function() {
			if ($scope.data.newTitle !== '' &&
				...
				$scope.data.newTitle !== null) {
				if ($scope.data.newDescrip !== '' &&
					$scope.data.newDescrip !== undefined &&
					$scope.data.newDescrip !== null) {
		...
		$scope.deleteTaskFromChecklist = function(task, checklist) {
			...
			confirmPopup.then(function(res) {
				if (res) {
					...
					// eUser.uTodo[eUser.uTodo.indexOf(checklist)]
					// .list
					// .splice(eUser.uTodo[eUser.uTodo.indexOf(checklist)]
					// .list.indexOf(item), 1);
					$scope.makeChangeTodoList();
				}
				// cancel
		...
		$scope.deleteTaskFromChecklistWithoutConfirm = function(task, list) {
			list.list.splice(list.list.indexOf(task), 1);

			if (task.done === false) {
				$scope.reduceUndoneTask(list);
			}
			// eUser.uTodo[eUser.uTodo.indexOf(list)]
			// .list.
			// splice(eUser.uTodo[eUser.uTodo.indexOf(list)]
			// .list.indexOf(item), 1);
			$scope.makeChangeTodoList();
		};
		...
		$scope.editTaskCut = function(task, cl) { // cl checklist
			$ionicPopup.show({
				...
				buttons: [{
					...
					text: 'Save',
					onTap: function( /*e*/ ) {
						eUser.uTodo[parseInt($scope.data.moveTo)]
							.list.push(task);
						$scope.deleteTaskFromChecklistWithoutConfirm(task, cl);
						if (task.done === false) {
							$scope.reduceUndoneTask(cl);
							$scope.unreduceUndoneTask(eUser
								.uTodo[parseInt($scope.data.moveTo)]);
						}
						$scope.makeChangeTodoList();
				...
				buttons: [{
					...
					onTap: function(e) {
						if ($scope.data.newTitle === '' ||
							...
							if ($scope.data.newDes === '' ||
								$scope.data.newDes === undefined) {
		...
		$scope.showChecklistActionSheet = function() {
			...
			$ionicActionSheet.show({
				...
				destructiveText: '<span class="easi-red"><b>Close</b><span>',
				titleText: '<h3 class="easi-gray">' +
					eUser.uTodo[$scope.viewList].listName +
					'</h3>',
				cancelText: '<span class="easi-gray">Cancel<span>',
				cancel: function() {
					// TODO cancel code here
